...
import { QuackMessage, MessageStatus } from '../types/index.js';
...
export class Dispatcher {
  private store: QuackStore;
  private pollInterval: number;
  private intervalId: ReturnType<typeof setInterval> | null = null;
  private webhookBaseUrls: Record<string, string> = {};
  ...
  private async poll() {
    try {
      ...
      for (const inboxName of inboxes) {
        ...
        const approved = messages.filter((m: QuackMessage) => m.status === 'approved');
  ...
  private async dispatch(message: QuackMessage) {
  ...
  async dispatchNow(messageId: string): Promise<boolean> {
