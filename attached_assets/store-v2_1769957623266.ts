 ...
 * 
 * UPDATED: Now integrates with session-registry for tracking conversation state
 */
...
export function sendMessage(req: SendMessageRequest, fromAgent: string): QuackMessage {
  ...
  // Determine thread status based on control message
  let threadStatus: 'active' | 'completed' | 'abandoned' | undefined;
  if (controlInfo.type === 'CONVERSATION_END') {
  ...
  try {
    ...
    // Session registry might not be initialized yet, that's OK
    console.log(`📋 Session tracking skipped: ${e}`);
  }
